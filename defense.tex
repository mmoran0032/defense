...
\begin{frame}[fragile]{Capture Reactions}
    ...
    Stellar energy production depends on the interior conditions of the
    star, such as the isotopic enrichment and temperature/pressure

    Hydrogen burning: $({\rm p},\gamma)$ reactions
    \begin{itemize}
        \item low mass energy production through $pp$-chains
        \item massive stars core and shell burning through CNO,
            NeNa, and MgAl cycles
    \end{itemize}

    Helium burning: $(\alpha,\gamma)$ reactions
    \begin{itemize}
        \item \react{\mnuc{12}{C}}{\alpha}{\gamma}{\mnuc{16}{O}}
            determines C/O ratio
        \item AGB stars during He burning stages are key source of
            neutrons for $s$-process (
                \react{\mnuc{13}{C}}{\alpha}{\rm n}{\mnuc{16}{O}},
                \react{\mnuc{22}{Ne}}{\alpha}{\rm n}{\mnuc{25}{Mg}}
                )
...
\begin{frame}[fragile]{Radiative Capture}
    ...
    \begin{itemize}
        \item large background count rates (target impurities,
            background radiation)
        \item limited by HPGe detector efficiency
        \item potentially low count rates
    \end{itemize}
...
\begin{frame}[fragile]{Particle Selection}
    ...
    \begin{alertblock}{Magnetic Selection}
        ...
        % \]
        selects a single momentum: $p = B\rho q$
    \end{alertblock}
    \begin{alertblock}{Electric Selection}
        % \[
        %     \frac{T}{q} = \frac{E\rho}{2}
        % \]
        selects a single energy: $T = E\rho(q/2)$
    \end{alertblock}
...
\begin{frame}[fragile]{Angular Acceptance Measurement}
    ...
    \begin{itemize}
        ...
        \item we will attempt to transport all $\alpha$s to detector
    \end{itemize}
...
\begin{frame}[fragile]{The Future of St.\ George}
    ...
    St.\ George can be used for a restricted subset of experiments

    Ability to fine-tune the separator and verify its properties over a
    range of $B\rho$ and $E\rho$ values essential for future experiments

    Diagnostic equipment and procedures developed to be applied to
    future separators (SECAR)

    Final parts of the St.\ George system (HIPPO supersonic jet gas
